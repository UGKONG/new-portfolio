...
// FormData 리턴 함수
export const useForm = (data = {}, files = null) => {
  console.log('useForm (data): ', data);
  console.log('useForm (files): ', files);
  if (typeof(data) != 'object') return console.warn('data is not object!!');

  let keys = Object.keys(data);
  if (keys.length == 0) return console.warn('data keys length is 0!!');
  ...
  if (files) {
    ...
    let values = Object.values(files[key]);
...
export const useForEach = (obj, fn) => {
